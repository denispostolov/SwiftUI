//
//  ContentView.swift
//  WeSplitChallenge
//
//  Created by Denis Postolov on 08/08/23.
//

import SwiftUI

struct ContentView: View {
    
    @State private var checkAmount = 0.0
    @State private var numberOfPeople = 2
    @State private var tipPercentage = 20
    @FocusState private var amountIsFocused: Bool
    
    // Challenge 4
    let currencyFormatter =  FloatingPointFormatStyle<Double>.Currency.currency(code: Locale.current.currency?.identifier ?? "EUR")
    
    
    // a computed tuple property in order to solve Challenge 2
    var totalAmount: (totalPerPerson: Double, grandTotal: Double) {
        
        let peopleCount = Double(numberOfPeople + 2) // handling the offset introduced by the ForEach in number of people Picker
        
        let tipSelection = Double(tipPercentage)
        
        let tipValue = checkAmount / 100 * tipSelection
        
        let grandTotal = checkAmount + tipValue
        
        let amountPerPerson = grandTotal / peopleCount
        
        return (amountPerPerson, grandTotal)
    }
    
    var body: some View {
        NavigationView {
            Form {
                Section {
                    
                    TextField("Amount", value: $checkAmount, format:  currencyFormatter)
                        .keyboardType(.decimalPad)
                        .focused($amountIsFocused)
                    
                    Picker("Number of people", selection: $numberOfPeople){
                        ForEach(2..<100){
                            Text("\($0) people")
                        }
                    }
                    
                } header: {
                    Text("Specify the check amount and the number of people that will split the check")
                }
                
                Section {
                    Picker("Tip selection", selection: $tipPercentage){
                        // Challenge 3
                        ForEach(0..<101, id: \.self){
                            Text($0, format: .percent)
                        }
                        
                    }
                    
                } header: {
                    Text("How much tip do you want to leave?")
                }
                
                // Challenge 2
                Section {
                    Text(totalAmount.grandTotal, format:  currencyFormatter)
                } header: {
                    Text("Total amount for the check")
                }
                
                Section {
                    Text(totalAmount.totalPerPerson, format:
                    currencyFormatter)
                } header: {
                    // Challenge 1
                    Text("Per person will be an amout of: ")
                }
                
                
            }
            .navigationTitle("WeSplit")
            .toolbar {
                ToolbarItemGroup(placement: .keyboard){
                    Spacer() // Flexible Space
                    
                    Button("Done") {
                        amountIsFocused = false
                    }
                }
            }
        }
        
    }
}

struct ContentView_Previews: PreviewProvider {
    static var previews: some View {
        ContentView()
    }
}
